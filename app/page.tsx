...
export default function Home() {
  ...
  return <main className="min-h-screen"><nav className="mx-auto flex max-w-6xl items-center justify-between px-6 py-6"><Link href="/" className="text-xl font-black tracking-tight">cutout<span className="text-indigo-600">.ai</span></Link><div className="flex items-center gap-5"><Link href="/pricing" className="hidden text-sm font-medium text-slate-600 hover:text-slate-900 sm:block">Pricing</Link><a href="#faq" className="hidden text-sm font-medium text-slate-600 hover:text-slate-900 sm:block">FAQ</a><AuthButton /></div></nav>
    <section className="mx-auto max-w-5xl px-6 pb-20 pt-12 text-center"><div className="mx-auto mb-5 inline-flex rounded-full bg-indigo-50 px-4 py-2 text-sm font-semibold text-indigo-700">Free online tool · Sign in with Google to get started</div><h1 className="text-5xl font-black tracking-tight text-slate-950 sm:text-7xl">Image background<br/><span className="text-indigo-600">remover</span></h1><p className="mx-auto mt-6 max-w-xl text-lg leading-8 text-slate-600">Remove backgrounds from images automatically and download a transparent PNG in seconds.</p>
    ...
    </section><section className="border-y border-slate-200 bg-white py-16"><div className="mx-auto grid max-w-5xl gap-8 px-6 text-center sm:grid-cols-3">{[['01','Upload','Choose a photo from your device.'],['02','Remove','Our image-processing service removes the background.'],['03','Download','Save a transparent PNG instantly.']].map(([n,t,d]) => <div key={n}><div className="mx-auto mb-4 flex h-10 w-10 items-center justify-center rounded-full bg-indigo-100 text-sm font-black text-indigo-700">{n}</div><h2 className="font-bold text-slate-900">{t}</h2><p className="mt-2 text-sm text-slate-500">{d}</p></div>)}</div></section><section id="faq" className="mx-auto max-w-3xl px-6 py-16"><h2 className="text-center text-3xl font-black">Frequently asked questions</h2><div className="mt-8 space-y-4">{[['Is this image background remover free?','You can start with a limited free allowance. Paid plans are available for more monthly removals.'],['What happens to my image?','Images are sent to the background removal service only for processing and are not stored by this website after processing.'],['What format do I get?','Successful results are returned as transparent PNG files.']].map(([q,a]) => <details key={q} className="rounded-2xl border border-slate-200 bg-white p-5"><summary className="cursor-pointer font-bold">{q}</summary><p className="mt-3 text-sm leading-6 text-slate-600">{a}</p></details>)}</div></section><footer className="border-t border-slate-200 py-8 text-center text-sm text-slate-500"><p>Questions? <a href="mailto:support@videomaker.tools" className="font-semibold text-indigo-600">support@videomaker.tools</a></p><div className="mt-3 flex justify-center gap-4"><Link href="/privacy" className="hover:text-slate-900">Privacy Policy</Link><Link href="/terms" className="hover:text-slate-900">Terms of Service</Link><Link href="/acceptable-use" className="hover:text-slate-900">Acceptable Use</Link></div><p className="mt-3">Images are processed temporarily and are not persistently stored.</p></footer></main>;
}
